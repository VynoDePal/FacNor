...
def calculate_invoice_totals(lines: list[TaxLineInput]) -> InvoiceTaxTotals:
    ...
    return InvoiceTaxTotals(
        lines=line_totals,
        total_excluding_tax=sum((line.total_excluding_tax for line in line_totals), Decimal("0")),
        total_tax=sum((line.total_tax for line in line_totals), Decimal("0")),
        total_including_tax=sum((line.total_including_tax for line in line_totals), Decimal("0")),
    )
