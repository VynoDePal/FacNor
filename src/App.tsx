...
import { AuthUser, Client, ClientPayload, Invoice, createClient, createInvoice, downloadInvoicePdf, fetchClients, fetchCurrentUser, fetchInvoices, login } from './api';
import './styles.css';
...
function App() {
  ...
  async function downloadPdf(invoice: Invoice) {
    if (!token) return;
    setDownloadingInvoiceId(invoice.id); setError(''); setSuccess('');
    try {
      const filename = await downloadInvoicePdf(token, invoice);
      setSuccess(`Téléchargement de ${filename} lancé.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Téléchargement PDF impossible'); }
