...
def test_update_client_to_b2b_validates_business_identifiers(client):
    ...
    headers = auth_headers(user["access_token"])
    created = client.post(
        "/clients", headers=headers, json={"name": "Prospect", "address": "1 rue A"}
    ).json()
...
def test_clients_are_scoped_to_authenticated_owner(client):
    ...
    created = client.post(
        "/clients", headers=first_headers, json={"name": "Privé", "address": "1 rue A"}
    )
    assert created.status_code == 201
    client_id = created.json()["id"]

    assert (
        client.get(f"/clients/{client_id}", headers=second_headers).status_code == 404
    )
    assert (
        client.put(
            f"/clients/{client_id}", headers=second_headers, json={"name": "Vol"}
        ).status_code
        == 404
    )
    assert (
        client.delete(f"/clients/{client_id}", headers=second_headers).status_code
        == 404
    )
